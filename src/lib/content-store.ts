...
import { blobEnvNames, createBlobStore, findBlobCredentials } from './stores/blob-store';
 ...
 */
const blobCredentials = findBlobCredentials();
const store: ContentStore = blobCredentials ? createBlobStore(blobCredentials) : createFileStore();
...
export const contentStoreWarning: string | null =
  process.env.VERCEL && !blobCredentials
    ? 'Content cannot be saved on Vercel until a Blob store is connected: open the project on vercel.com → Storage → Create Database → Blob → connect it to this project, then redeploy. ' +
      `Expected BLOB_READ_WRITE_TOKEN or BLOB_STORE_ID in the Production environment; BLOB variables visible to this deployment: ${blobEnvNames().join(', ') || 'none'}.`
    : null;
